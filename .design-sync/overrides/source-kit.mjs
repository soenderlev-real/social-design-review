// forked from design-sync lib/source-kit.mjs — synth-entry mode wrote
// `export * from "<file>"` for every src file, which ES-module semantics
// never forward `export default` through. This repo's components are all
// `export default function Name() {}`, so the original code discovered them
// (deriveComponentsFromSrc recovers default-export names) but the actual
// runtime bundle exported nothing — window.<GLOBAL>.<Name> was undefined for
// every component ([BUNDLE_EXPORT] + [RENDER_ERRORS] "Element type is
// invalid"). Patched: the synthesized entry now also emits
// `export { default as <Name> } from "<file>"` for any file with a default
// export, using the same real-name recovery as deriveComponentsFromSrc.
//
...
// `export * from "<file>"` never forwards `export default` (ES-module
// semantics) — recover each file's real default-export name (same recovery
// deriveComponentsFromSrc uses) so the synthesized entry can re-export it
// explicitly via `export { default as <Name> } from "<file>"`. Also reports
// whether the file has any real *named* export worth forwarding via
// `export *` at all — a bare entry/bootstrap file (imports only, top-level
// side-effect code, zero exports — e.g. this repo's src/main.jsx, which
// calls `ReactDOM.createRoot(document.getElementById('root'))` at module
// scope) has neither, and blindly `export * from`-ing it anyway still
// evaluates the module and runs that side effect, which throws outside a
// real page with a `#root` element and aborts the WHOLE bundle before
// window.<GLOBAL> is ever assigned (surfaces as [BUNDLE_EXPORT] failing for
// every component, not just the bootstrap file). Skip files with neither.
function exportInfo(project, p) {
  const sf = project.addSourceFileAtPathIfExists(p);
  if (!sf) return { hasDefault: null, hasNamed: false };
  const decls = sf.getExportedDeclarations();
  const def = decls.get('default');
  const real = def ? def.map((d) => d.getName?.()).find((n) => n && n !== 'default') : null;
  const hasDefault = real && /^[A-Z][A-Za-z0-9]*$/.test(real) ? real : null;
  const hasNamed = [...decls.keys()].some((k) => k !== 'default');
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const entryLines = comps.flatMap((p) => {
      const { hasDefault, hasNamed } = exportInfo(entryProject, p);
      if (!hasDefault && !hasNamed) return []; // bare bootstrap/entry file — nothing to forward, don't evaluate it
      const lines = [];
      if (hasNamed) lines.push(`export * from ${JSON.stringify(p)};`);
      if (hasDefault) lines.push(`export { default as ${hasDefault} } from ${JSON.stringify(p)};`);
      return lines;
    });
