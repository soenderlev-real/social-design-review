...
export default function Header({ onLogoClick, onInstructionsClick, onReadingListClick, showBack }) {
  ...
  return (
    <header className="bg-white border-b-2 border-dark sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {showBack && (
            <button
              onClick={onLogoClick}
              className="border-2 border-dark px-3 py-1 text-sm hover:bg-dark hover:text-white transition-colors"
              aria-label="Back"
            >
          ...
          >
            ...
            />
            <span className="ml-3 text-darker text-sm hidden sm:inline">
        ...
        <button
          ...
          className="sm:hidden border-2 border-dark px-3 py-1 hover:bg-dark hover:text-white transition-colors"
        >
      ...
      {/* Mobile menu panel */}
      {menuOpen && (
        <nav className="sm:hidden border-t-2 border-dark bg-white animate-fade-in">
          {navItems.map(item =>
            item.href ? (
              <a
                key={item.label}
                ...
                className="block px-6 py-3 text-sm text-dark border-b-2 border-lighter last:border-b-0 hover:bg-light transition-colors"
              >
                {item.label}
              </a>
            ) : (
              <button
                ...
                className="block w-full text-left px-6 py-3 text-sm text-dark border-b-2 border-lighter last:border-b-0 hover:bg-light transition-colors"
              >
                {item.label}
              </button>
            )
