...
function renderPrompt(text) {
  ...
  lines.forEach((raw, i) => {
    ...
    if (line.startsWith('### ')) { flush(); elements.push(<h5 key={i} className="text-xs uppercase tracking-widest text-darker mt-6 mb-2">{line.slice(4)}</h5>); return; }
    if (line.startsWith('## ')) { flush(); elements.push(<h4 key={i} className="text-base text-dark mt-8 mb-2 first:mt-0">{line.slice(3)}</h4>); return; }
  ...
  return (
    <div className="border-2 border-dark -mt-[2px] first:mt-0 bg-white">
      <button onClick={() => setOpen(o => !o)} className="w-full text-left px-5 py-4 flex items-center gap-4 hover:bg-light transition-colors">
        <div className="flex-shrink-0 w-8 h-8 border-2 border-dark flex items-center justify-center text-xs text-darker">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {meta
              ? <span className={`px-1.5 py-0.5 text-xs border ${meta.badge}`}>{meta.label}</span>
              : <span className="px-1.5 py-0.5 text-xs border border-rb-orange text-rb-orange">Core</span>}
          </div>
          <p className="text-xs text-darker truncate">{concept.shortDesc}</p>
        </div>
        {open ? <ChevronUp size={16} className="text-darker flex-shrink-0" /> : <ChevronDown size={16} className="text-darker flex-shrink-0" />}
      </button>
      ...
      {open && (
        <div className="border-t-2 border-dark px-5 pb-6 pt-5 animate-fade-in">
          ...
          <div className="mt-6 p-4 bg-light border-2 border-dark">
            <h5 className="text-xs uppercase tracking-widest text-darker mb-3 flex items-center gap-2">
    ...
    <div className="mt-12">
      <div className="flex items-start gap-4 mb-4">
        ...
        <div>
          ...
          {subtitle && <p className="text-xs text-darker mt-0.5">{subtitle}</p>}
        </div>
      </div>
      {children}
    </div>
  ...
  return (
    <section className="min-h-[70vh] flex items-center justify-center px-6">
      <form onSubmit={handleSubmit} className="w-full max-w-xs border-2 border-dark bg-white p-8">
        ...
        <h1 className="text-dark text-lg mb-1">Instructions</h1>
        <p className="text-xs text-darker mb-6">This reference page is password protected.</p>
        <input
          type="password"
          value={value}
          ...
          autoFocus
          className="w-full px-4 py-3 border-2 border-dark bg-white text-dark placeholder-muted focus:outline-none focus:bg-rb-blue-tint transition-colors text-sm mb-3"
        />
        {error && <p className="text-xs text-rb-red mb-3">Incorrect password.</p>}
        <button type="submit" className="w-full bg-dark text-white border-2 border-dark py-3 text-sm uppercase tracking-widest hover:bg-darker transition-colors">
          Unlock
        </button>
      </form>
    </section>
    ...
    <div className="animate-fade-in">
      ...
      <section className="bg-white border-b-2 border-dark">
        <div className="max-w-5xl mx-auto px-6 py-16">
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-2 mt-8 pt-6 border-t-2 border-dark text-xs text-darker">
            <span><span className="text-dark">Modes</span> &nbsp;Review &middot; Design Workshop</span>
            <span><span className="text-dark">Dimensions</span> &nbsp;13 (10 core + 3 meta)</span>
            <span><span className="text-dark">Consumed by</span> &nbsp;src/utils/analyzeWithAI.js</span>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto px-6 py-16">
          <Section number="1" title="Analysis System Prompt" subtitle="Sent with every dimension in Review Mode, against a live platform URL" accent="bg-dark">
            ...
            <div className="mt-4 p-4 bg-light border-2 border-dark">
              <h5 className="text-xs uppercase tracking-widest text-darker mb-3">Required output — seven fixed sections, in order</h5>
      ...
      <section className="border-b-2 border-dark bg-white">
        <div className="max-w-5xl mx-auto px-6 py-16">
          <Section number="2" title="Design System Prompt" subtitle="Sent in Design Workshop Mode, against a platform concept instead of a live URL" accent="bg-rb-blue-shade">
            <div className="border-2 border-dark p-6 sm:p-8">
              {renderPrompt(DESIGN_SYSTEM_PROMPT)}
            </div>
            <div className="mt-4 p-4 bg-light border-2 border-dark">
              <h5 className="text-xs uppercase tracking-widest text-darker mb-3">Required output — five fixed sections, in order</h5>
              <div className="flex flex-wrap gap-2">
      ...
      <section className="bg-white">
        <div className="max-w-5xl mx-auto px-6 py-16">
          <Section number="3" title="Core Dimensions" subtitle="Each dimension's promptContext, darkPatterns, lightPatterns, and keyQuestions from framework.js" accent="bg-rb-orange-shade">
          ...
          <Section number="4" title="Meta-Dimensions" subtitle="Holistic passes across the core dimensions above — Enable, Grow, Protect" accent="bg-rb-green-shade">
            <div className="space-y-0">
